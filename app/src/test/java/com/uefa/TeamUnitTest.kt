...
class TeamUnitTest {
    ...
    @Test
    fun onlyOneGoalKeeperInStartingLine() {

        val line = team.getTeamLine(UefaTeam.Member.Role.GOALKEEPER,UefaTeam.Member.Status.STARTING_LINE_UP)
        Assert.assertEquals(line.size, 1)
    }

    @Test
    fun twoForwardsInStartingLine() {

        val line = team.getTeamLine(UefaTeam.Member.Role.FORWARD,UefaTeam.Member.Status.STARTING_LINE_UP)
        Assert.assertEquals(line.size, 2)
    }
}